...
use sessionbus_core::{
    sha256_hex, Artifact, ArtifactKind, BusEvent, ContextPack, CreateArtifactRequest,
    CreateDecisionRequest, CreateSessionRequest, Decision, PackProfile, RedactionPolicy, Session,
    SessionStatus, UpdateSessionStatusRequest, WorkspaceInfo,
};
...
impl ApiClient {
    ...
    async fn list_events(&self, session_id: Option<&str>) -> Result<Vec<BusEvent>> {
...
fn workspace_watch_artifact(workspace: &Path) -> Result<CreateArtifactRequest> {
...
fn print_workspace() -> Result<()> {
    print!("{}", workspace_text()?);
    Ok(())
}

fn workspace_text() -> Result<String> {
    let workspace = detect_workspace()?;
...
fn git_output_in<const N: usize>(args: [&str; N], cwd: &Path) -> Result<Option<String>> {
...
fn workspace_session_path(root: &str) -> PathBuf {
...
fn write_workspace_session(root: &str, session_id: &str) -> Result<()> {
...
fn clear_active_session_if_matches(session_id: &str) -> Result<()> {
    let path = active_session_path();
...
fn clear_workspace_session_if_matches(root: &str, session_id: &str) -> Result<()> {
    let path = workspace_session_path(root);
    let Ok(current) = fs::read_to_string(&path) else {
        return Ok(());
    };
